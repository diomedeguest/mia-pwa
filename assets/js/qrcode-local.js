...
function localRequire(id){
  ...
  var module={exports:{}}; cache[id]=module; modules[id](module,module.exports,localRequire); return module.exports;
}
var QRCode = localRequire('index');
var EC = localRequire('QRErrorCorrectLevel');
...
function createSvg(text, options){
  ...
  for(var r=0;r<count;r++){
    for(var c=0;c<count;c++){
      if(qr.isDark(r,c)) path.push('M'+(c+quiet)+' '+(r+quiet)+'h1v1h-1z');
    }
  }
  return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+size+' '+size+'" role="img" aria-label="QR code" shape-rendering="crispEdges">'+
    '<rect width="100%" height="100%" fill="#fff"/>'+ '<path d="'+escapeXml(path.join(''))+'" fill="#111"/></svg>';
}
global.DiomedeQR={createSvg:createSvg};
})(window);
